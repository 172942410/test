            ...
            while (true) {
                try {   
                	while(true){                		
		        		...
		        		Log.i(TAG,"收到串口数据2: receiveBuffer:"+receiveBuffer);
		        		//发送显示
	    				mHandler.obtainMessage(BluetoothSet.MESSAGE_READ, receiveBuffer.length(), -1, receiveBuffer.getBytes())
	    	        		.sendToTarget();		    				
	    				sleep(10L);
	    				receiveBuffer = "";
		        		/*
		        		...
		        		if (receiveBuffer.endsWith("\r\n")){
		        			...
		        			Log.i(TAG,"收到串口数据2: strArray:"+strArray.toString());
		        			for(String stemp:strArray){
		        				Log.i(TAG,"收到串口数据3: stemp:"+stemp);
		        				//发送显示
